        ...
        if(pid > 0) {
            if(pid == 1 || pid == 2 || pid == 3 || pid == 4) {
                ...
                $.getJSON('/misc/city?pid=' + pid, function(json){
                    $("option", city).remove();
                    var option = '<option value="-1">==涓嶉檺鍩庡競==</option>';
                    city.append(option);
...
function msg_info(msg, okfunc, cancelfunc) {
    if($('#msgInfoModal').length < 1) {
        $('body').append('<div id="msgInfoModal" class="modal fade">' +
            ...
            '<div class="modal-header"><h4 class="modal-title">娓╅Θ鎻愰啋</h4></div>'+
            '<div class="modal-body"><strong></strong></div>'+
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-default" data-dismiss="modal">鍙栨秷</button>'+
            '<button type="button" class="btn btn-primary" id="confirmdelete">纭畾</button>'+
            '</div>'+
            '</div>'+
            '</div>'+
            '</div>');
...
function msg_error(msg, btnfunc) {
    if($('#msgErrorModal').length < 1) {
        $('body').append('<div id="msgErrorModal" class="modal fade">' +
            ...
            '<h4 class="modal-title">閿欒鎻愮ず</h4>'+
            '</div>'+
            '<div class="modal-body"></div>'+
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-default" data-dismiss="modal">纭畾</button>'+
            '</div>'+
            '</div>'+
            '</div>'+
            '</div>');
...
function msg_alert(msg, btnfunc) {
    if($('#msgErrorModal').length < 1) {
        $('body').append('<div id="msgErrorModal" class="modal fade">' +
            ...
            '<div class="modal-content">' +
            '<div class="modal-header">'+
            '<h4 class="modal-title">娓╅Θ鎻愮ず</h4>'+
            '</div>'+
            '<div class="modal-body"></div>'+
            '<div class="modal-footer">'+
            '<button type="button" class="btn btn-default" data-dismiss="modal">纭畾</button>'+
